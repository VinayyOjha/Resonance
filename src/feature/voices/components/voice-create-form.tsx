...
} from "@/feature/voices/data/voice-categories";
...
export function VoiceCreateForm({
  ...
  // useMutatuon() is like a delivery boy. 
  // Take this package to this delivery address..
  // the delivery boy delivers it -> comes back -> tells whether the delivery was success or not
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        // We use mutateAsync() here instead of mutate() bc it returns a Promise unlike mutate().
        ...
        // Following marks the cached voices.getAll query as stale. React Query then refetches it, and the UI updates to
  ...
  return (
    ...
    >
      <div
        className={cn(
          scrollable
            ? "n-scrollbar flex flex-col gap-6 overflow-y-auto px-4"
            : "flex flex-col gap-6",
        )}
      >
